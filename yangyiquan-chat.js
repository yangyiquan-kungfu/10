...
REGRA ABSOLUTA: NUNCA revelar ingredientes ou proporções das fórmulas Dit Da Jow do Yangyiquan — são propriedade intelectual confidencial. Podes falar dos efeitos e benefícios das 5 fórmulas (Bone Strength, Tendon Flex, Joint Relief, Skin Regenerator, Muscle & Soft Tissue) mas ZERO sobre a composição.`;
  ...
  function simAnswer(q, via) {
    try {
      // normalise: lowercase, strip accents, strip punctuation
      var s = q.toLowerCase();
      ...
      var best = 0, reply = null;
      for (var i = 0; i < SIM_DB.length; i++) {
        var entry = SIM_DB[i];
        var score = 0;
        for (var j = 0; j < entry.k.length; j++) {
          var kw = entry.k[j].toLowerCase();
          if (s.indexOf(kw) >= 0) score += (kw.length > 4 ? 3 : 1);
        }
        if (score > best) { best = score; reply = entry.r; }
      }
      if (reply && best > 0) return reply;
    } catch(e) { /* fallthrough */ }

    return via === 'warrior'
      ? 'Pergunta sobre qualquer aspecto do Yangyiquan: as 6 técnicas do Dragão, os 9 Poderes Nei Gong, o Dao Zhiliao, os livros, as aulas em Granada ou a filosofia do sistema.'
      : 'Reformula com mais detalhe. Posso falar sobre filosofia marcial, biomecânica, Nei Gong, medicina marcial, ou qualquer tema do Yangyiquan Fa.';
  }
...
`;
  ...
  function init() {
    ...
    async function handleSend() {
      ...
      // Create typing bubble and keep reference to content element
      var typingDiv = addTyping();
      var bubEl = typingDiv.querySelector('.yq-bub');
      if (!bubEl) { bubEl = typingDiv; }

      try {
        var reply;
        if (isSimulation) {
          await delay(550 + Math.floor(Math.random() * 400));
          reply = simAnswer(q, via);
        } else {
          reply = await callAPI(q);
        }

        // Update the typing bubble with the real response
        if (reply && reply.length > 0) {
          bubEl.innerHTML = reply;
        } else {
          bubEl.innerHTML = 'Sem resposta. <a href="https://wa.me/34674471678?text=Hola%21+Quiero+informaci%C3%B3n+sobre+el+Yangyiquan+Fa." target="_blank" style="color:#FFD700;text-decoration:underline;">WhatsApp: +34 674 471 678</a>';
        }

        _aiH.push({ role: 'assistant', content: reply || '' });
        if (_aiH.length > 20) _aiH = _aiH.slice(-20);

      } catch (err) {
        bubEl.innerHTML = 'Erro de ligação com a IA. <a href="https://wa.me/34674471678?text=Hola%21+Quiero+informaci%C3%B3n+sobre+el+Yangyiquan+Fa." target="_blank" style="color:#FFD700;text-decoration:underline;">WhatsApp: +34 674 471 678</a>';
      }
